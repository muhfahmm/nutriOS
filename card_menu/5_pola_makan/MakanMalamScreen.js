...
export default function MakanMalamScreen({ navigation }) {
  ...
  const fetchMealData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const dinnerSched = schedules.find(s => s.meal_type === 'dinner');
        if (dinnerSched && dinnerSched.meal_time !== '-') {
          const [h, m] = dinnerSched.meal_time.split('.');
          setHour(h);
          setMinute(m);
          setNotifEnabled(dinnerSched.notif_enabled === 1);
        }
      }
    } catch (e) {
  ...
  const handleSaveTime = async () => {
    ...
    try {
      ...
      if (notifEnabled) {
        ...
        Alert.alert('Berhasil', `Waktu pengingat makan malam diubah ke ${formattedHour}.${formattedMinute} dan Notifikasi Diaktifkan!`);
      } else {
        await Notifications.cancelScheduledNotificationAsync('meal-dinner');
        ...
        Alert.alert('Berhasil', `Waktu pengingat makan malam diubah ke ${formattedHour}.${formattedMinute} (Notifikasi Nonaktif)`);
      }
    } catch (e) {
      console.log(e);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Menu Makan Malam</Text>
        <View style={{ width: 24 }} />
      </View>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.card}>
            <View style={styles.cardHeaderRow}>
              ...
              <Text style={styles.sectionTitle}>Atur Waktu Pengingat Makan Malam</Text>
            </View>

            {/* Toggle Switch Notifikasi */}
            <View style={styles.notifToggleRow}>
              <View style={styles.notifToggleTextCol}>
                <Text style={styles.notifToggleLabel}>Aktifkan Notifikasi Makan Malam</Text>
                <Text style={styles.notifToggleDesc}>Dapatkan push notification pada jam makan malam</Text>
              </View>
              <Switch
                value={notifEnabled}
